...
runInElement'
  ...
  -> Effect { push :: push -> Effect Unit, unsubscribe :: Effect Unit }
runInElement' elt psh mksn = do
  ffi <- makeFFIDOMSnapshot
  { push, event } <- create
  let evt = deku elt (mksn push event) effectfulDOMInterpret
  unsubscribe <- subscribe evt \i -> i ffi
  push psh
  pure {push, unsubscribe}
...
runInBody' push go = do
  ...
  maybe (pure { push: mempty, unsubscribe: mempty}) (\elt -> runInElement' elt push go) (toElement <$> b')
